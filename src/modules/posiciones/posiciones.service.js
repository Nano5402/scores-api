...
exports.getByTorneo = async (torneoId) => {
  ...
  const [torneoRows] = await db.query('SELECT deporte FROM torneos WHERE id = ? LIMIT 1', [torneoId]);
  ...
  const [partidos] = await db.query(
    'SELECT id, ' + col1 + ', ' + col2 + ', ganador FROM partidos WHERE torneo_id = ? AND estado = ?',
    [torneoId, 'finalizado']
  );
  ...
  if (isTenis) {
    ...
    const [equipos] = await db.query(
      'SELECT id, nombre FROM equipos_padel WHERE id IN (?)',
      [idsArr]
    );
    equipos.forEach((e) => {
